  ...
  for (const OperatorCode* opcode : *opcodes) {
    ...
    if (status != kTfLiteOk) {
      ...
      if (!opcode->custom_code()) {
        error_reporter_->Report(__FILE__, __LINE__,
...
TfLiteStatus InterpreterBuilder::ParseNodes(
  ...
  for (int i = 0; i < operators->size(); ++i) {
    ...
    if (index < 0 || index >= flatbuffer_op_index_to_registration_.size()) {
      error_reporter_->Report(__FILE__, __LINE__,"Missing registration for opcode_index %d\n",
                              index);
      status = kTfLiteError;
      continue;
    }
    ...
    if (registration == nullptr) {
      error_reporter_->Report(__FILE__, __LINE__,"Skipping op for opcode_index %d\n", index);
      status = kTfLiteError;
      continue;
    }
...
TfLiteStatus InterpreterBuilder::ParseSparsity(
  ...
  if (src_sparsity->traversal_order() == nullptr ||
    ...
    error_reporter_->Report(__FILE__, __LINE__,"Invalid sparsity parameter.");
    return kTfLiteError;
  }
...
TfLiteStatus InterpreterBuilder::ParseTensors(
  ...
  for (int i = 0; i < tensors->size(); ++i) {
    ...
    if (ParseQuantization(src_quantization, &quantization, dims) != kTfLiteOk) {
      error_reporter_->Report(__FILE__, __LINE__,"Tensor %d has invalid quantization parameters.",
                              i);
      status = kTfLiteError;
    }
    ...
    if (buffer_ptr) {
      ...
      if (ParseSparsity(src_sparsity, &sparsity) != kTfLiteOk) {
        error_reporter_->Report(__FILE__, __LINE__,"Tensor %d has invalid sparsity parameters.",
                                i);
        status = kTfLiteError;
      }
      ...
      if (subgraph->SetTensorParametersReadOnly(
        ...
        error_reporter_->Report(__FILE__, __LINE__,"Tensor %d is invalidly specified in schema.\n",
                                i);
        status = kTfLiteError;
      }
    } else {
...
TfLiteStatus InterpreterBuilder::operator()(
  ...
  if (!model_) {
    error_reporter_->Report(__FILE__, __LINE__,"Null pointer passed in as model.");
  ...
  if (BuildLocalIndexToRegistrationMapping() != kTfLiteOk) {
    error_reporter_->Report(__FILE__, __LINE__,"Registration failed.\n");
    return cleanup_and_error();
  }
  ...
  if (subgraphs->size() == 0) {
    error_reporter_->Report(__FILE__, __LINE__,"No subgraph in the model.\n");
    return cleanup_and_error();
  }
