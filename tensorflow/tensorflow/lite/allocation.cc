...
FileCopyAllocation::FileCopyAllocation(const char* filename,
  ...
  if (!file) {
    error_reporter_->Report(__FILE__, __LINE__, "Could not open '%s'.", filename);
    return;
  }
  struct stat sb;
...
#define FILENO(_x) fileno(_x)
  ...
  if (fstat(FILENO(file.get()), &sb) != 0) {
    error_reporter_->Report(__FILE__, __LINE__, "Failed to get file size of '%s'.", filename);
    return;
  }
#undef FILENO
  ...
  if (!buffer) {
    error_reporter_->Report(__FILE__, __LINE__, "Malloc of buffer to hold copy of '%s' failed.",
                            filename);
    return;
  }
  size_t bytes_read =
  ...
  if (bytes_read != buffer_size_bytes_) {
    error_reporter_->Report(__FILE__, __LINE__, "Read of '%s' failed (too few bytes read).",
                            filename);
    return;
  }
...
MemoryAllocation::MemoryAllocation(const void* ptr, size_t num_bytes,
  ...
  if ((reinterpret_cast<uintptr_t>(ptr) & 0x3) != 0) {
    ...
    TF_LITE_REPORT_ERROR(error_reporter, __FILE__, __LINE__, 
