...
class Subgraph {
 public:
  ...
  void ReportError(const char *file, int line, const char* format, ...);
 ...
 private:
  ...
  void ReportErrorImpl(const char *file, int line, const char* format, va_list args);
  ...
  // Entry point for C node plugin API to report an error.
  static void ReportErrorC(TfLiteContext* context, const char *file, int line, const char* format, ...);
