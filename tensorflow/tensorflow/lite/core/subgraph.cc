...
TfLiteStatus ForbiddenContextFunction(TfLiteContext* context, ...) {
  context->ReportError(context,__FILE__, __LINE__,
...
TfLiteStatus Subgraph::AllocateTensors() {
  ...
  if (!consistent_) {
    ReportError(__FILE__, __LINE__,"AllocateTensors() called on inconsistent model.");
    return kTfLiteError;
  }
...
TfLiteStatus Subgraph::AddNodeWithParameters(
  ...
  if (state_ == kStateInvokableAndImmutable) {
    ReportError(__FILE__, __LINE__,"AddNodeWithParameters is disallowed when graph is immutable.");
    return kTfLiteError;
  }
...
TfLiteStatus Subgraph::ResizeInputTensor(int tensor_index,
  ...
  if (graph_is_immutable && !delegates_applied) {
    ReportError(__FILE__, __LINE__,"ResizeInputTensor is disallowed when graph is immutable.");
    return kTfLiteError;
  }
  ...
  if (op_reg.prepare == nullptr) {
    ...
    if (IsUnresolvedCustomOp(op_reg)) {
      if (IsFlexOp(op_reg.custom_name)) {
        ...
        ReportError(__FILE__, __LINE__,"Encountered unresolved custom op: %s.",
...
TfLiteStatus Subgraph::Invoke() {
  if (!consistent_) {
    ReportError(__FILE__, __LINE__,"Invoke called on model that is not consistent.");
    return kTfLiteError;
  }

  TfLiteStatus status = kTfLiteOk;
  if (state_ == kStateUninvokable) {
    ReportError(__FILE__, __LINE__,"Invoke called on model that is not ready.");
    return kTfLiteError;
  } else if (memory_planner_ && !memory_planner_->HasNonPersistentMemory()) {
    ReportError(__FILE__, __LINE__,"Non-persistent memory is not available.");
    return kTfLiteError;
  }
  ...
  for (int execution_plan_index = 0;
    ...
    if (check_cancelled_func_ != nullptr &&
        check_cancelled_func_(cancellation_data_)) {
      ReportError(__FILE__, __LINE__,"Client requested cancel during Invoke()");
      return kTfLiteError;
    }
...
void Subgraph::ReportErrorImpl(const char *file, int line, const char* format, va_list args) {
  error_reporter_->Report(file, line, format, args);
}

void Subgraph::ReportErrorC(TfLiteContext* context, const char *file, int line, const char* format, ...) {
  va_list args;
  ...
  // (this function is static).
  f->ReportErrorImpl(file, line, format, args);
  va_end(args);
}

// Entry point for C node plugin API to report an error.
void Subgraph::ReportError(const char *file, int line, const char* format, ...) {
  va_list args;
  va_start(args, format);
...
TfLiteStatus Subgraph::SetTensorParametersReadWrite(
  ...
  if (type == kTfLiteString) {
    if (is_variable) {
      ...
      ReportError(__FILE__, __LINE__,"String variable tensor isn't supported.");
      return kTfLiteError;
    }
...
TfLiteStatus Subgraph::ResizeTensorImpl(TfLiteTensor* tensor,
  ...
  if (tensor->allocation_type == kTfLiteArenaRw ||
    ...
    // of fixed size.
    TfLiteIntArrayFree(new_size);
    ReportError(__FILE__, __LINE__, "Attempting to resize a fixed-size tensor.");
    return kTfLiteError;
  }
  return kTfLiteOk;
}
...
void Subgraph::UseNNAPI(bool enable) {
  ...
  if (applied_nnapi_delegate_ && !enable) {
    ReportError(__FILE__, __LINE__, "Attempting to disable NNAPI delegate after it's applied.");
  } else {
