...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (num_inputs != 5 && num_inputs != 6) {
    context->ReportError(context,__FILE__, __LINE__, "Found NMS op with invalid num inputs: %d",
                         NumInputs(node));
    return kTfLiteError;
  }
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (is_soft_nms) {
    ...
    if (soft_nms_sigma < 0) {
      context->ReportError(context,__FILE__, __LINE__, "Invalid sigma value for soft NMS: %f",
                           soft_nms_sigma);
      return kTfLiteError;
    }
