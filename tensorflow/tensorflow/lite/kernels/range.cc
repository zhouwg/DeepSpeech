...
TfLiteStatus ResizeOutput(TfLiteContext* context, const TfLiteTensor* start,
  ...
  switch (start->type) {
    ...
    default: {
      context->ReportError(context,__FILE__, __LINE__, "Unknown data type: %d", start->type);
      return kTfLiteError;
    }
  }
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (dtype != kTfLiteFloat32 && dtype != kTfLiteInt32) {
    context->ReportError(context,__FILE__, __LINE__, "Unknown index output data type: %s",
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (output->type) {
    ...
    default: {
      context->ReportError(context,__FILE__, __LINE__, "Unsupported data type: %d", output->type);
      return kTfLiteError;
    }
  }
  return kTfLiteOk;
