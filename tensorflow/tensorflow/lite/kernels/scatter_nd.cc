...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (updates->type) {
    ...
    case kTfLiteInt32:
      break;
    default:
      context->ReportError(context, __FILE__, __LINE__, "Updates of type '%s' are not supported by scatter_nd.",
          TfLiteTypeGetName(updates->type));
      return kTfLiteError;
  }
  if (indices->type != shape->type) {
    context->ReportError(context, __FILE__, __LINE__, "Indices and shape must have the same type.");
    return kTfLiteError;
  }
  ...
  if (IsConstantTensor(shape)) {
    switch (indices->type) {
      ...
      default:
        context->ReportError(
            context, __FILE__, __LINE__, "Indices of type '%s' are not supported by scatter_nd.",
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (indices->type) {
    ...
    default:
      context->ReportError(context, __FILE__, __LINE__, "Indices of type '%s' are not supported by scatter_nd.",
          TfLiteTypeGetName(indices->type));
      return kTfLiteError;
  }
}
