...
TfLiteStatus CalculateOutputShapeVector(TfLiteContext* context,
  ...
  for (int idx = 0; idx < NumDimensions(input); ++idx) {
    ...
    if (size_value < 0) {
      if (size_value != -1) {
        context->ReportError(context, __FILE__, __LINE__,  "Invalid size.");
        return kTfLiteError;
      }
      ...
      if (SizeOfDimension(input, idx) <
        ...
        context->ReportError(context, __FILE__, __LINE__, "Invalid begin and size.");
        return kTfLiteError;
      }
    }
...
TfLiteStatus ResizeOutputShape(TfLiteContext* context,
  ...
  if (begin->type == kTfLiteInt32) {
    ...
    context->ReportError(context, __FILE__, __LINE__, "Type %d is currently not supported by Slice.", begin->type);
    return kTfLiteError;
  }
...
#define TF_LITE_SLICE(data_type, kernel_type)                                  \
  ...
  switch (input->type) {
    ...
    case kTfLiteString:
      ...
      break;
    default:
      context->ReportError(context, __FILE__, __LINE__, "Type %d is currently not supported by Slice.", input->type);
      return kTfLiteError;
  }
#undef TF_LITE_SLICE
  return kTfLiteOk;
