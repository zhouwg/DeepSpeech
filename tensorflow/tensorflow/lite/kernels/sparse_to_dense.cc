...
TfLiteStatus CheckDimensionsMatch(TfLiteContext* context,
  ...
  switch (NumDimensions(indices)) {
    ...
    default:
      context->ReportError(
          context, __FILE__, __LINE__, "Wrong indices dimensions %d, should be less than 3.",
...
TfLiteStatus ResizeOutputShape(TfLiteContext* context,
  ...
  if (output_shape->type == kTfLiteInt32) {
    ...
    context->ReportError(context,__FILE__, __LINE__, "Dense shape type %d not supported.",
                         output_shape->type);
    return kTfLiteError;
  }
}
