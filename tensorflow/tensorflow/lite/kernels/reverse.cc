...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type != kTfLiteInt32 && input->type != kTfLiteFloat32 &&
    ...
    context->ReportError(context,__FILE__, __LINE__, "Type '%s' is not supported by reverse.",
                         TfLiteTypeGetName(input->type));
    return kTfLiteError;
  }

  if (axis->type != kTfLiteInt32) {
    context->ReportError(context,__FILE__, __LINE__, "Axis Type '%s' is not supported by reverse.",
  ...
  if (NumElements(axis) > 1) {
    context->ReportError(context,__FILE__, __LINE__, "Current does not support more than 1 axis.");
  }
