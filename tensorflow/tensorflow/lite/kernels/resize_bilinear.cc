...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (params->half_pixel_centers && params->align_corners) {
    context->ReportError(context, __FILE__, __LINE__, "If half_pixel_centers is True, align_corners must be False.");
    return kTfLiteError;
  }
...
#undef TF_LITE_RESIZE_BILINEAR
    ...
    context->ReportError(context,__FILE__, __LINE__, "Output type is %d, requires float.",
                         output->type);
    return kTfLiteError;
  }
