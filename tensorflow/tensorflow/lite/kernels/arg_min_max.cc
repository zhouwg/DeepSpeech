...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (params->output_type) {
    ...
    case kTfLiteInt64:
      ...
      break;
    default:
      context->ReportError(context,__FILE__, __LINE__, "Unknown index output data type: %d",
                           params->output_type);
      return kTfLiteError;
  }
...
#define TF_LITE_ARG_MIN_MAX(data_type, axis_type, output_type) \
  ...
  if (axis->type == kTfLiteInt32) {
    switch (output->type) {
      ...
      default:
        context->ReportError(context,__FILE__, __LINE__, "Only int32 and int64 are supported currently, got %s.",
