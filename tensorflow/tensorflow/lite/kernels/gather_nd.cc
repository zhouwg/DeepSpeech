...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (params->type) {
    ...
    case kTfLiteString:
      break;
    default:
      context->ReportError(context, __FILE__, __LINE__, "Params of type '%s' are not supported by gather_nd.",
  ...
  switch (indices->type) {
    ...
    case kTfLiteInt32:
      break;
    default:
      context->ReportError(context, __FILE__, __LINE__, "Indices of type '%s' are not supported by gather_nd.",
  ...
  if (params_rank < 1) {
    context->ReportError(context,__FILE__, __LINE__, "Params must be at least a vector.");
    return kTfLiteError;
  }
  if (indices_rank < 1) {
    context->ReportError(context,__FILE__, __LINE__, "Indices must be at least a vector.");
    return kTfLiteError;
  }
  if (indices_nd > params_rank) {
    context->ReportError(context, __FILE__, __LINE__, "Index innermost dimension length must be <= params rank.");
    return kTfLiteError;
  }
