...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (node->inputs->size == 24) {
    ...
    context->ReportError(context, __FILE__, __LINE__, "The LSTM Full kernel expects 20 or 24 inputs. Got %d inputs",
        node->inputs->size);
    return kTfLiteError;
  }
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input_to_output_weights->type) {
    ...
    default:
      context->ReportError(context,__FILE__, __LINE__, "Type %d is not currently supported.",
