...
TfLiteStatus ResizeOutputTensors(TfLiteContext* context, TfLiteNode* node,
  ...
  if (size_splits->type == kTfLiteInt32) {
    ...
    context->ReportError(context, __FILE__, __LINE__, "size_splits only support type int32|int64.");
    return kTfLiteError;
  }
...
#define TF_LITE_SPLIT_V(scalar)                                     \
  ...
  switch (op_context.input->type) {
    ...
    default:
      context->ReportError(context, __FILE__, __LINE__, "Type %s currently not supported.",
