    ...
    switch (tensor_type) {
      ...
      default:
        context_->ReportError(
            context_, __FILE__, __LINE__, "Failed to add NN API tensor: type %s is not supported.",
...
TfLiteStatus NNAPIDelegateKernel::Init(TfLiteContext* context,
  ...
  if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI12 &&
    ...
    if (nnapi_devices_.empty()) {
      context->ReportError(
          context, __FILE__, __LINE__,"NNAPI delegate requested but no accelerators available.");
      return kTfLiteError;
    }
  }
