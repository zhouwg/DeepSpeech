...
OpBuilder* GraphBuilder::CreateOpBuilderFromTfLiteOp(int op_type,
  ...
  switch (op_type) {
    ...
    default:
      context_->ReportError(context_, __FILE__, __LINE__, "Op not supported: %d", op_type);
      return nullptr;
  }
}
...
OpBuilder* GraphBuilder::AddConstNodeWithData(const int shape[], char* data,
  ...
  if (error != 0) {
    context_->ReportError(context_, __FILE__, __LINE__,  "Error adding const node with shape id: %d",
                          (int)builders_.size());
    return nullptr;
  }
...
OpBuilder* GraphBuilder::AddConstNodeWithData(int tensor_id,
  ...
  if (error > 0) {
    context_->ReportError(
        context_, __FILE__, __LINE__, "Failed to add const node for tensor with id: %d", tensor_id);
    return nullptr;
  }
